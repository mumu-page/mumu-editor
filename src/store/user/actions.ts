...
import { Selectors, UserState } from "./state";

/**
 * Actions that actually modify the todo content
 */
export class TodoReducer extends ImmerReducer<UserState> {
    selectors = new Selectors(this.draftState);

    addTodo(payload: { id: string }) {
        // this.draftState.user[payload.id] = {
 ...
 */
export class TodoLifecycleReducer extends ImmerReducer<UserState> {
    selectors = new Selectors(this.draftState);
